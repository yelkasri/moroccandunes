...
foreach ($blocks as $block) {
    ...
    $block_classes[$block] .= ($this['config']->get("block.{$block}.block-fullheight")) ? ' uk-height-viewport' : '';
    $block_classes[$block] .= ($this['config']->get("block.{$block}.block-middle")) ? ' tm-block-middle' : '';
    $block_classes[$block] .= ($this['config']->get("block.{$block}.block-contrast")) ? ' uk-contrast' : '';
    $block_classes[$block] .= ($this['config']->get("block.{$block}.block-texture")) ? ' tm-block-texture' : '';

    $styles["block.$block"] = '';

    if ($this['config']->get("block.{$block}.block-image", false)) {
        $styles["block.$block"] = 'style="background-image: url(\'' . $this['config']->get("block.{$block}.block-image") . '\');"';
...
$navbar = "";
switch ($this['config']->get('navbar_mode', '0')) {
    case '1':
        // Sticky
        $sticky = 'data-uk-sticky="{'.implode(', ', $sticky).'}"';
        $navbar = "uk-navbar-attached";
        break;
    ...
    case '2':
        ...
        $sticky[] = "clsactive: 'uk-active uk-navbar-attached'";
        $sticky   = 'data-uk-sticky="{'.implode(', ', $sticky).'}"';
        $body_classes .= ' tm-navbar-transparent';
        $navbar = $this['config']->get('navbar-invert', 0) ? $this['config']->get('navbar-invert') : '';
        break;
